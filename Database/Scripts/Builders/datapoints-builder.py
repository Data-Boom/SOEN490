...
for key in data:
    ...
    datasetfile = open("txt with datapoints/" + key + ".txt", "r")
    ...
    for variable in temp:
        ...
        if (
            "initial temperature" not in variable
            and "initial pressure" not in variable
            and "equivalence ratio" not in variable
        ):
            ...
            if "(" in variable:
                ...
                data[key]["data"]["variables"].append(
                    {
                        "name": variableName,
                        "repr": "",
            ...
            # otherwise, there is no unit and the units must be labeled "N/A" for the dataset entry later
                ...
                variableNames.append(variableName)
                data[key]["data"]["variables"].append(
                    {"name": variableName, "repr": "", "units": "N/A", "value": []}
                )
        ...
        for x in range(len(variableNames)):
            ...
            for each in range(len(data[key]["data"]["variables"])):
                if data[key]["data"]["variables"][each]["name"] == attempt:
                    if not isinstance(
                        data[key]["data"]["variables"][each]["value"], list
                    ):
                        data[key]["data"]["variables"][each]["value"] = []
                    data[key]["data"]["variables"][each]["value"].append(
                        float(points[x].replace("\n", "").strip())
                    )

    # Same as above but for the case 2.
    for each in data[key]["data"]["variables"]:
        if each["name"] == "initial temperature" and not isinstance(
            each["value"], list
        ):
            if "-" in each["value"]:
            ...
            if "-" in each["value"]:
                ...
                each["value"] = [float(each["value"])] * data[key]["data"]["count"]
        elif each["name"] == "equivalence ratio" and not isinstance(
            each["value"], list
        ):
            if "-" in each["value"]:
                each["details"] = each["value"]
