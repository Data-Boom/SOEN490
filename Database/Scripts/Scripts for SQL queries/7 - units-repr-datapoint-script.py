...
for x in data:  # Loop through each dataset
    ...
    for y in data[x]["data"]["variables"]:  # Loop each variable array
        ...
        if y["units"] not in list_of_units and y["units"] != "" and y["units"] != "N/A":
            ...
            file.write(
                " INSERT INTO `databoom_db`.`units` (`id`, `name`, `units`) VALUES ("
                + str(unitCount)
                + ', "'
                + y["units"]
                + '", "'
        ...
        if (
            y["repr"] not in list_of_representations
            and y["repr"] != ""
            and y["repr"] != "N/A"
        ):
            ...
            file.write(
                " INSERT INTO `databoom_db`.`representations` (`id`, `repr`) VALUES ("
                + str(reprCount)
                + ', "'
                + y["repr"]
                + '");\n'
        ...
        datapointsFile.write(
            " INSERT INTO `databoom_db`.`datapoints`(`datasetId`, `name`, `values`, `unitsId`, `representationsId`) VALUES ("
