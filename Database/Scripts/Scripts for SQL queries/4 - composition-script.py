...
for x in data:  # Loop through dataset
    ...
    for y in data[x]["materials"]:  # Loop through compositions
        if y["composition"] not in list_of_composition:
            list_of_composition[y["composition"]] = {
                "name": y["composition"],
                "id": count,
            }  # Insert composition into dictionary
            dataset_materials_material[x] = {
                "compositionId": count,
                "datasetId": data[x]["datasetId"],
            }  # Insert compositionId and datasetId

            # Build 3 sets of txt files with SQL queries
            # Composition query
            file.write(
                " INSERT INTO `databoom_db`.`composition` (`id`, `composition`) VALUES ("
                + str(count)
                + ', "'
                + y["composition"]
                + '");\n'
            ...
            # Material query
            materialFile.write(
                " INSERT INTO `databoom_db`.`material` (`id`, `compositionId`, `details`) VALUES ("
                + str(count)
                + ", "
                + str(count)
                + ', "'
            ...
            dmmFile.write(
                " INSERT INTO `databoom_db`.`dataset_materials_material` (`datasetId`, `materialId`) VALUES ("
                + str(data[x]["datasetId"])
                + ", "
                + str(count)
                + ");\n"
            )
            ...
            dataset_materials_material[x] = {
                "compositionId": list_of_composition[y["composition"]]["id"],
                "datasetId": data[x]["datasetId"],
            }
            ...
            dmmFile.write(
                ...
                + str(data[x]["datasetId"])
                + ", "
                + str(list_of_composition[y["composition"]]["id"])
                + ");\n"
            )
