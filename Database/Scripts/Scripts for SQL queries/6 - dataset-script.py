...
for x in data:
    file.write(
        " INSERT INTO `databoom_db`.`dataset` (`id`, `name`, `datatypeId`, `publicationId`, `categoryId`, `subcategoryId`, `comments`) VALUES ("
        + str(data[x]["datasetId"])
        + ', "'
        + x
        + '", 1, '
        + str(data[x]["reference"])
        + ", "
        + str(data[x]["categoryId"])
        + ", "
        + str(data[x]["subcategoryId"])
        + ', "'
        + data[x]["dataset-comment"]
        + '");\n'
    )
